...
abstract class ImageProcessing {
    ...
    private static int colordecoderYUV(byte[] yuv420sp, int width, int height, int type) {
     ...
     */
    static double colordecoderRGB(byte[] yuv420sp, int width, int height, int type) {
        if (yuv420sp == null) return 0;
        final int frameSize = width * height;

        int sum = colordecoderYUV(yuv420sp, width, height, type);
